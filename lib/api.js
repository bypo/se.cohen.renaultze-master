...
class RenaultApi {
    ...
    supportChargeMode() {
        ...
        if (modelCode === 'XJA1VP' || modelCode === 'XBG1VE' || modelCode === 'XCB1VE') {
...
//      }
    ...
    async _kamereonGetMegane(id_token, path, version) {

        console.log('_kamereonGetMegane:' + path)

        return axios.get(
            this.configuration.kamereon_url + '/commerce/v1/accounts/' + this.settings.accountId + "/kamereon/kapi/vehicles/" + this.settings.vin + "/" + path + "?country=" + this.settings.country, {
            headers: {
                'x-gigya-id_token': id_token,
                'apikey': this.configuration.kamereon_api_key
            }
